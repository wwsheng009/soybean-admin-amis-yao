...
const Vue = (function (exports) {
  ...
  const isRef$1 = val => {
    return Boolean(val && val.__v_isRef === true);
  };
  ...
  function isReactive(value) {
    ...
    return Boolean(value && value.__v_isReactive);
  }
  function isReadonly(value) {
    return Boolean(value && value.__v_isReadonly);
  }
  function isShallow(value) {
    return Boolean(value && value.__v_isShallow);
  }
  function isProxy(value) {
    return value ? Boolean(value.__v_raw) : false;
  }
    ...
    process(n1, n2, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized, internals) {
      ...
      if (n1 == null) {
        ...
        if (disabled) {
          ...
          } else if (n2.props && n1.props && n2.props.to !== n1.props.to) {
  ...
  function defineComponent(options, extraOptions) {
    return isFunction(options)
      ? // #8236: extend call and options.name access are considered side-effects
        ...
        /* @__PURE__ */ (() => ({ name: options.name, ...extraOptions, setup: options }))()
      : options;
  }
...
- expected on client:`,
  ...
  const isAsyncWrapper = i => Boolean(i.type.__asyncLoader);
  /*! #__NO_SIDE_EFFECTS__ */
  // @__NO_SIDE_EFFECTS__
  function defineAsyncComponent(source) {
    ...
    return defineComponent({
      ...
      setup() {
        ...
        const error = ref();
        const delayed = ref(Boolean(delay));
        if (delay) {
          setTimeout(() => {
            delayed.value = false;
          }, delay);
...
If this is a native custom element, make sure to exclude it from component resolution via compilerOptions.isCustomElement.`
  ...
  const publicPropertiesMap =
    ...
    /* @__PURE__ */ extend(/* @__PURE__ */ Object.create(null), {
      ...
      $nextTick: i => (i.n ||= nextTick.bind(i.proxy)),
  ...
  const RuntimeCompiledPublicInstanceProxyHandlers = /* @__PURE__ */ {
  ...
  function mergeModels(a, b) {
    ...
    return { ...normalizePropsOrEmits(a), ...normalizePropsOrEmits(b) };
  }
...
If you want to remount the same app, move your app creation logic into a factory function and create fresh app instances for each mount - e.g. \`const createMyApp = () => createApp(App)\``
  ...
  function hasInjectionContext() {
    return Boolean(currentInstance || currentRenderingInstance || currentApp);
  }
  ...
  function baseCreateRenderer(options, createHydrationFns) {
    ...
    const patch = (
      ...
      namespace = void 0,
      slotScopeIds = null,
      optimized = isHmrUpdating ? false : Boolean(n2.dynamicChildren)
    ) => {
      if (n1 === n2) {
        return;
      }
  ...
  function createSuspenseBoundary(
    ...
    const suspense = {
      ...
      registerDep(instance, setupRenderEffect, optimized2) {
        const isInPendingSuspense = Boolean(suspense.pendingBranch);
  ...
  const normalizeRef = ({ ref, ref_key, ref_for }) => {
    ...
    return ref != null
      ? isString(ref) || isRef(ref) || isFunction(ref)
        ? { i: currentRenderingInstance, r: ref, k: ref_key, f: Boolean(ref_for) }
        : ref
      : null;
  };
...
Component that was made reactive: `,
  ...
  function guardReactiveProps(props) {
    if (!props) return null;
    return isProxy(props) || isInternalObject(props) ? { ...props } : props;
  }
  ...
  function createSetupContext(instance) {
    ...
    };
    {
      ...
      return Object.freeze({
        get attrs() {
          return (attrsProxy ||= new Proxy(instance.attrs, attrsProxyHandlers));
        },
        get slots() {
          return (slotsProxy ||= getSlotsProxy(instance));
        },
        get emit() {
  ...
  function getTransitionInfo(el, expectedType) {
    ...
    const hasTransform =
      type === TRANSITION$1 && /\b(transform|all)(,|$)/.test(getStyleProperties(`${TRANSITION$1}Property`).toString());
    return {
      type,
      timeout,
      propCount,
  ...
  function patchAttr(el, key, value, isSVG, instance, isBoolean = isSpecialBooleanAttr(key)) {
    ...
    } else if (value == null || (isBoolean && !includeBooleanAttr(value))) {
...
Expected function or array of functions, received type ${typeof value}.`
  ...
  const TransitionGroupImpl = /* @__PURE__ */ decorate({
    ...
    props: /* @__PURE__ */ { ...TransitionPropsValidators, tag: String, moveClass: String },
    setup(props, { slots }) {
      const instance = getCurrentInstance();
      const state = useTransitionState();
      let prevChildren;
  ...
  const rendererOptions = /* @__PURE__ */ { patchProp, ...nodeOps };
  let renderer;
...
Make sure to use the production build (*.prod.js) when deploying for production.`
  ...
  class Tokenizer {
    constructor(stack, cbs) {
      ...
      this.entityStart = 0;
      /**
       * Some behavior, eg. when decoding entities, is done while we are in another state. This keeps track of the other
       * state type.
       */
      this.baseState = 1;
    ...
    /**
     * Generate Position object with line / column information using recorded newline positions. We know the index is
     * always going to be an already processed index, so all the newlines up to this index should have been recorded.
     */
    getPos(index) {
      let line = 1;
      let column = index + 1;
    ...
    /**
     * When we wait for one specific character, we can speed things up by skipping through the buffer until we find it.
     *
...
Use a v-bind binding combined with a v-on listener that emits update:x event instead.`,
  ...
  function isStaticArgOf(arg, name) {
    return Boolean(arg && isStaticExp(arg) && arg.content === name);
  }
  ...
  function onCloseTag(el, end, isImplied = false) {
    ...
    if (tokenizer.inSFCRoot) {
      if (el.children.length) {
        el.innerLoc.end = { ...el.children[el.children.length - 1].loc.end };
      } else {
        el.innerLoc.end = { ...el.innerLoc.start };
      }
...
`,
  ...
  function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
    ...
    if (onComponentSlot) {
      ...
      slotsProperties.push(
        createObjectProperty(arg || createSimpleExpression('default', true), buildSlotFn(exp, void 0, children, loc))
      );
    }
  ...
  function isNonWhitespaceContent(node) {
    ...
    return node.type === 2 ? Boolean(node.content.trim()) : isNonWhitespaceContent(node.content);
  }
  ...
  function baseCompile(source, options = {}) {
    ...
    const resolvedOptions = { ...options, prefixIdentifiers };
  ...
  function compileToFunction(template, options) {
    ...
    if (!opts.isCustomElement && typeof customElements !== 'undefined') {
      opts.isCustomElement = tag => Boolean(customElements.get(tag));
    }
